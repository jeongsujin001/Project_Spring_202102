...
<c:forEach var="cb" items="${commentList }">
		...
		<c:if test="${cb.comment_del == 0}">
			${cb.comment_content}			
				...
				<c:if test="${! empty sessionScope.id }">
					<c:if test="${sessionScope.id eq cb.comment_id}">
						...
						<a href='<c:url value="/board/commentDelete?comment_num=${cb.comment_num}&comment_bnum=${cb.comment_bnum}"/>' class="bu_gray_s" onclick='return button_event();'>삭제</a>
						</span>
						<span style="float:right;">
   		...
   		<c:if test="${cb.comment_del != 0}">
			<span class="del">삭제된 댓글입니다</span></div>
					...
					<form name="comment" id="comment" action='<c:url value="/board/commentUpdate"/>' method="post" style="margin-top:0px !important; padding-top:0px !important;">
					...
					<form name="comment" id="comment" action='<c:url value="/board/commentReWrite"/>' method="post" style="margin-top:0px !important; padding-top:0px !important;">
					<input type="hidden" name="comment_id" value="${sessionScope.id }">
					<input type="hidden" name="comment_bnum" value="${num }">
					<input type="hidden" name="comment_num" value="${cb.comment_num }">
	...
	<form name="comment" id="comment" action='<c:url value="/board/commentWrite"/>' method="post" style="margin-top:0px !important; padding-top:0px !important;">
		<input type="hidden" name="comment_id" value="${sessionScope.id }">
		<input type="hidden" name="comment_bnum" value="${num }">
			<div class="comment_box_wrap">
...
<c:if test="${! empty sessionScope.id }">
	<c:if test="${sessionScope.id eq bb.name}">
			<input type="button" value="글수정" class="bu_gray_l" onclick="location.href='<c:url value="/board/update?num=${bb.num}"/>'">
			<input type="button" value="글삭제" class="bu_gray_l" onclick="location.href='<c:url value="/board/delete?num=${bb.num}"/>'">
